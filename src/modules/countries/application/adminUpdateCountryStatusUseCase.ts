...
import { adminUpdateCountryStatusInputSchema, countryDtoResponseSchema } from "./dtos/country.dto";
import { ApplicationError } from "../../../shared/domain/error";

export class AdminUpdateCountryStatusUseCase extends UseCase<any, any> {
    protected inputSchema = adminUpdateCountryStatusInputSchema;
    ...
    protected async implementation(input: { id: number, status: boolean }): Promise<any> {
        ...
        return await this.countryRepository.update(id, { is_active: status });
    }
}
