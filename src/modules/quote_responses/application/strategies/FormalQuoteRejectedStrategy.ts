...
/**
 * FormalQuoteRejectedStrategy
 * Buyer rejects the formal PDF quote, returning to negotiation.
 * Status transitions back to `negotiating`.
 */
export class FormalQuoteRejectedStrategy implements QuoteRevisionStrategy {
    readonly action = 'formal_quote_rejected';
    ...
    async execute(params: QuoteActionParams): Promise<QuoteResponse> {
        ...
        return this.repo.updateWithRevision(
            quoteResponseId,
            {
                ...
                formal_quote_url: null, // Clear the rejected PDF
                rejection_reason: payload?.rejection_reason ?? null,
            },
            {
                action: this.action,
                actorCompanyId,
                snapshot: { rejection_reason: payload?.rejection_reason ?? null },
            }
        );
    }
}
